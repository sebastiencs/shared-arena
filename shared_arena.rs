...
use super::page::{BLOCK_PER_PAGE, Block};
use super::shared_page::SharedPage;
...
pub struct SharedArena<T: Sized> {
    free: Arc<Atomic<SharedPage<T>>>,
    page_list: Atomic<SharedPage<T>>,
    npages: usize,

    // last_found: CacheAligned<AtomicUsize>,
    // pages: CacheAligned<Atomic<Vec<NonNull<Page<T>>>>>,
...
impl<T: Sized> SharedArena<T> {
    ...
    ) -> (NonNull<SharedPage<T>>, NonNull<SharedPage<T>>)
    {
        let last = SharedPage::<T>::new(arena_free_list, std::ptr::null_mut());
        let mut previous = last;

        for _ in 0..npages - 1 {
            let previous_ptr = unsafe { previous.as_mut() };
            let page = SharedPage::<T>::new(arena_free_list, previous_ptr);
            previous = page;
        }
    ...
    fn alloc_new_page(&mut self, guard: &Guard) -> NonNull<SharedPage<T>> {
        let len = self.npages;
        ...
        loop {
            let current = self.free.load(Relaxed, guard);
            last_ref.next_free = Atomic::from(current);

            if self.free.compare_and_set_weak(
                current, first, (Release, Relaxed), guard
            ).is_ok() {
                break;
            }
        }

        last_ref.next = Atomic::from(self.page_list.load(Relaxed, guard));
        self.page_list.store(first_ref, Relaxed, guard);
    ...
    fn with_single_writer(&self, guard: &epoch::Guard, fun: impl Fn()) {
        let _writer_guard = match WriterGuard::new(&self.writer) {
            Some(guard) => guard,
            _ => return
        };

        if !self.free.load(Acquire, guard).is_null() {
    ...
    fn find_place(&self) -> (NonNull<SharedPage<T>>, NonNull<Block<T>>) {

        let guard = epoch::pin();

        loop {

            while let Some(page) = unsafe { self.free.load(Acquire, &guard).as_mut() } {

                if let Some(block) = page.acquire_free_block() {
                    return (NonNull::from(page), block);
                }

                let next = page.next_free.load(Relaxed, &guard);
                ...
                if self.free.compare_and_set(page, next, (AcqRel, Relaxed), &guard).is_ok() {
                    page.in_free_list.store(false, Release);
                }
            }

            self.with_single_writer(&guard, || {
                self.alloc_new_page(guard);
            });
        }
    ...
    pub fn with_capacity(cap: usize) -> SharedArena<T> {
        let npages = ((cap.max(1) - 1) / BLOCK_PER_PAGE) + 1;
        let free = Arc::new(Atomic::from(std::ptr::null()));
        ...
        SharedArena {
            npages,
            free,
            page_list: Atomic::from(first as *const _),
            writer: CacheAligned::new(AtomicBool::new(false)),
        }
    }
